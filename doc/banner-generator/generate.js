...
const goCode = [
  ...
  line(span(' ')),
  line(span('reg := tygor.NewRegistry()')),
  line(span('news := reg.Service('), span('"News"', colors.string), span(')')),
  line(span('news.Register('), span('"List"', colors.string), span(', tygor.Unary(ListNews).Method('), span('"GET"', colors.string), span('))')),
  line(span('news.Register('), span('"Create"', colors.string), span(', tygor.Unary(ListNews))')),
  line(span(' ')),
  line(span('if', colors.keyword), span(' *gen {')),
  line(span('  tygorgen.Generate(reg, &tygorgen.Config{OutDir: '), span('"./rpc/client"', colors.string), span('})')),
  line(span('} ', colors.keyword), span('else', colors.keyword), span(' {')),
  line(span('  http.ListenAndServe('), span('":8080"', colors.string), span(', reg.Handler())')),
  line(span('}')),
];
